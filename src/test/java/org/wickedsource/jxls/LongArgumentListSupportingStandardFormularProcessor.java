 ...
 */
public class LongArgumentListSupportingStandardFormularProcessor implements FormulaProcessor {
    @Override
        ...
        for (CellData formulaCellData : formulaCells) {
            ...
            for (int i = 0; i < targetFormulaCells.size(); i++) {
                ...
                for (Map.Entry<CellRef, List<CellRef>> cellRefEntry : targetCellRefMap.entrySet()) {
                    ...
                    // Replace SUM function with concatenation via "+" to support more than 255 values
                    if(targetFormulaString.startsWith("SUM")){
                        if(hasMoreThan255Parameters(targetFormulaString)) {
                            targetFormulaString = targetFormulaString.replaceAll("^SUM\\(", "");
                            targetFormulaString = targetFormulaString.replaceAll("\\)$", "");
                            targetFormulaString = targetFormulaString.replaceAll(",", "+");
                        }
                    }
                }
    ...
    private boolean hasMoreThan255Parameters(String functionString){
        String[] fragments = functionString.split(",");
        return fragments.length >= 255;
    }
